...
/// Focus / phase / scenario consistency matrix for CoachV6 selection.
final class CoachV6SelectionMatrixTests: XCTestCase {

    private let colors = (r: 0.2, g: 0.6, b: 0.9)

    private let forbiddenPostStrength: Set<CoachV6ScenarioKey> = [
        .postStrengthImmediate, .postStrengthSettled, .eveningAfterStrength
    ]

    private let forbiddenPostEndurance: Set<CoachV6ScenarioKey> = [
        .postEnduranceImmediate, .postEnduranceSettled, .eveningAfterEndurance
    ]

    private let forbiddenPostRacket: Set<CoachV6ScenarioKey> = [
        .postRacketImmediate, .postRacketSettled, .eveningAfterRacket
    ]

    private let forbiddenPostMindfulRecovery: Set<CoachV6ScenarioKey> = [
    ...
    func testCompletedWalkBeforeUpcomingCoreDoesNotResolvePostStrengthImmediate() throws {
        ...
        let focus = CoachV6FocusResolver.resolve(input: input)
    ...
    func testCompletedWalkWithWorkoutTypeIsNotClassifiedAsStrength() {
        ...
        XCTAssertEqual(CoachV6ActivityClassifier.type(for: walk), .walk)
        XCTAssertEqual(CoachV6ActivityClassifier.family(for: walk), .recovery)
    }
    ...
    func testUpcomingCyclingWithLowRecoveryUsesPreSessionNotPost() {
        ...
        XCTAssertEqual(CoachV6FocusResolver.resolve(input: input).source, .upcoming)
    ...
    func testPostStrengthImmediateCopyMentionsCompletedStrength() throws {
        let now = date(hour: 10, minute: 30)
        let core = completedActivity(title: "Core", endedMinutesAgo: 12, durationMinutes: 45, relativeTo: now)
        let result = CoachV6Engine.evaluate(input: makeInput(now: now, activities: [core], brainHour: 10))
        let pack = try XCTUnwrap(result.copyPack)
    ...
    func testActiveEnduranceCopyMentionsUpcomingRide() throws {
        let now = date(hour: 13, minute: 0)
        let cycling = upcomingActivity(title: "Cycling", at: now.addingTimeInterval(90 * 60), icon: "figure.outdoor.cycle")
        let result = CoachV6Engine.evaluate(input: makeInput(now: now, activities: [cycling], brainHour: 13))
        let pack = try XCTUnwrap(result.copyPack)
    ...
    func testWalkScenarioCopyMentionsWalk() throws {
        ...
        let result = CoachV6Engine.evaluate(input: makeInput(now: now, activities: [walk], brainHour: 9))
        let pack = try XCTUnwrap(result.copyPack)

        let walkScenarios: Set<CoachV6ScenarioKey> = [.walkLightDay, .walkRecoveryAction, .walkAfterHeavyLoad]
    ...
    private struct SelectionExpectation {
        let expectedScenario: CoachV6ScenarioKey?
        let allowedScenarios: Set<CoachV6ScenarioKey>?
        let forbiddenScenarios: Set<CoachV6ScenarioKey>
        let expectedSource: CoachV6FocusSource?
        let expectedPhase: CoachV6SessionPhase?
        let expectedFamily: CoachV6ActivityFamily?
        let expectedType: CoachV6ActivityType?
    }

    private func assertSelection(
        now: Date,
        activities: [PlannedActivity],
        expectedScenario: CoachV6ScenarioKey? = nil,
        allowedScenarios: Set<CoachV6ScenarioKey>? = nil,
        forbiddenScenarios: Set<CoachV6ScenarioKey> = [],
        expectedSource: CoachV6FocusSource? = nil,
        expectedPhase: CoachV6SessionPhase? = nil,
        expectedFamily: CoachV6ActivityFamily? = nil,
        expectedType: CoachV6ActivityType? = nil,
    ...
    private func assertConsistentSelection(input: CoachInputSnapshot, result: CoachV6Engine.Result) {
        let focus = CoachV6FocusResolver.resolve(input: input)
        XCTAssertTrue(
            CoachV6ScenarioConsistency.isConsistent(selection: focus, scenario: result.scenario),
    ...
    private func makeInput(
        ...
        return CoachInputSnapshot(
            ...
            source: "CoachV6SelectionMatrixTests"
        )
    }
    ...
    private func joinedRussianCopy(
        _ pack: CoachV6CopyPack,
        bridge: CoachV6TabPresentationBridge.Result
    ) -> String {
        [
            bridge.coach.title,
