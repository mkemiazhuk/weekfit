...
final class CoachScenarioTests: XCTestCase {
    ...
    func testEnduranceHydrationBehindKeepsEnduranceScenario() {
        ...
        let result = CoachEngine.evaluate(input: input, focusActivity: cycling)
        ...
        XCTAssertEqual(
            result.scenario,
            CoachScenarioResolver.primaryScenarioIgnoringNutrition(result.context)
        )
    }
    ...
    func testHeavyDayWithTomorrowMorningWorkoutUsesTomorrowProtection() throws {
        ...
        XCTAssertTrue(CoachTabPresentationBridge.build(from: result) != nil)
    ...
    func testEveningAfterLongRideUsesCoachCopyPack() {
        ...
        XCTAssertNotNil(result.copyPack)
        XCTAssertNotNil(CoachTabPresentationBridge.build(from: result))

        let debug = CoachIntegrationDebug.resolve(from: result, usingCoach: true)
        XCTAssertTrue(debug.usingCoach)
    }
    ...
    func testDayLoadLivesOnlyInModifiers() {
        ...
        let fresh = CoachEngine.evaluate(input: freshInput, focusActivity: cycling)
        let heavy = CoachEngine.evaluate(input: heavyInput, focusActivity: cycling)
