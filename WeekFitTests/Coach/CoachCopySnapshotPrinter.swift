...
/// Manual QA printer for Coach Russian copy — test target only.
enum CoachCopySnapshotPrinter {

    static let logFileURL = URL(fileURLWithPath: "/tmp/WeekFitCoachCopySnapshots-RU.txt")
    ...
    private static func snapshot(
        ...
        input: CoachCopyBuildInput? = nil,
        variantLabel: String? = nil
    ) -> Snapshot? {
        let buildInput = input ?? CoachCopyQualityTests.baselineInput(for: scenario)
        guard let result = engineResult(from: buildInput),
              let bridge = CoachTabPresentationBridge.build(from: result) else {
            return nil
        }
        ...
        return Snapshot(
            ...
            todaySubtitle: bridge.todayMessage,
            coachHero: bridge.coachTitle,
    ...
    private static func engineResult(from input: CoachCopyBuildInput) -> CoachEngine.Result? {
        ...
        let context = CoachContext(
            ...
            activityState: input.activityState != .none
                ? input.activityState
                : activityState(for: input.scenario),
            sessionPhase: input.sessionPhase != .idle
                ? input.sessionPhase
                : sessionPhase(for: input.scenario),
            ...
            focusSource: input.focusSource != .idle
                ? input.focusSource
                : focusSource(for: input.scenario),
            minutesUntilStart: isPreSession(input.scenario) ? 12 : nil,
            minutesSinceEnd: input.minutesSinceEnd ?? (isPostSession(input.scenario) ? 8 : nil),
    ...
    private static func header() -> String {
        ...
        xcodebuild test -scheme WeekFit \\
          ...
          -only-testing:WeekFitTests/CoachCopySnapshotPrinterTests/testPrintAllRussianCopySnapshots
        """
    }
    ...
    private static func russianText(_ section: CoachCopySection) -> String {
        section.lines.map(\.russian).joined(separator: " ")
    }

    // MARK: - Scenario groups

    private static func scenarios(in group: PhaseGroup) -> [CoachScenarioKey] {
        switch group {
        case .dayLevel:
    ...
    private static func hydrationCriticalScenarios() -> [CoachScenarioKey] {
        [.duringEndurance, .duringRacket, .saunaActive]
    }

    private static func hydrationCriticalInput(for scenario: CoachScenarioKey) -> CoachCopyBuildInput {
        let base = CoachCopyQualityTests.baselineInput(for: scenario)
        return CoachCopyBuildInput(
            ...
            modifiers: CoachScenarioModifiers(
                ...
                lastCompletedActivityType: base.modifiers.lastCompletedActivityType
            ),
    ...
    private static func stackedRiskInput(
        ...
        return CoachCopyBuildInput(
            ...
            modifiers: CoachScenarioModifiers(
                ...
                lastCompletedActivityType: activityType
            ),
    ...
    private static func isPreSession(_ scenario: CoachScenarioKey) -> Bool {
        switch scenario {
    ...
    private static func isPostSession(_ scenario: CoachScenarioKey) -> Bool {
        switch scenario {
