...
| `testActiveActivityCannotHideOverloadPlanReplacement` | `planChallenge` / `trainingReadinessWarning` | `activeSession` / `activeActivity` | Overload replacement may be masked | **real regression** | Safety-critical: active session must not hide plan replacement when readiness is overloaded. |
| `testTodayPresentationUsesComposedFrameNarrative` | Overload frame: `"Replace the run"` + overload read | Prep frame: `"The run is close now"` + conservative open | **real regression** | Same overload scenario now renders prep-window story instead of plan-challenge replacement arc. |
