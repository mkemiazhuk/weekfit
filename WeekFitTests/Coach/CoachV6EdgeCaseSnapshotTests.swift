...
/// Ownership-conflict matrix for CoachV6 — see `CoachV6EdgeCaseMatrix.md`.
final class CoachV6EdgeCaseSnapshotTests: XCTestCase {
    ...
    func testEdgeCase04BPrime_autoFocusPicksUpcomingCoreOverIdleProtection() throws {
        ...
        assertExpectation(
            Expectation(
                ...
                badge: "ВНИМАНИЕ",
                todayTitle: "Нагрузка на пределе",
    ...
    func testEdgeCase08_stackedDayRiskOverridesPresentation() throws {
        ...
        assertExpectation(
            Expectation(
                ...
                todayTitle: "Нагрузка на пределе",
                stackedDayActiveRisk: true,
                conflictNote: "scenario key stays duringStrength — copy/title from stacked overlay"
            ),
            result: result,
            bridge: bridge
        )
    ...
    func testEdgeCase16A_enduranceActiveToPostNoFlicker() throws {
        ...
        let checkpoints: [(hour: Int, minute: Int, scenario: CoachV6ScenarioKey, markComplete: Bool)] = [
        ...
        var previous: CoachV6ScenarioKey?
    ...
    func testPrintEdgeCaseMatrix() {
        print(CoachV6EdgeCaseMatrixPrinter.renderHeader())
        print("See individual test logs above when running full CoachV6EdgeCaseSnapshotTests suite.")
        print("Log: \(CoachV6EdgeCaseMatrixPrinter.logFileURL.path)")
    }
    ...
    private func assertExpectation(
        ...
        result: CoachV6Engine.Result,
        bridge: CoachV6TabPresentationBridge.Result
    ) {
        let row = CoachV6EdgeCaseMatrixPrinter.format(
        ...
        print(row)
        CoachV6EdgeCaseMatrixPrinter.appendToLog(row)
    ...
    private func requireBridge(_ result: CoachV6Engine.Result) throws -> CoachV6TabPresentationBridge.Result {
        try XCTUnwrap(CoachV6TabPresentationBridge.build(from: result))
    }
    ...
    private func evaluate(
        ...
        let input = CoachInputSnapshot(
            ...
            source: "CoachV6EdgeCaseSnapshotTests"
        )
        return CoachV6Engine.evaluate(input: input, focusActivity: focus)
    }
    ...
    private func assertNoNutritionNarrative(in pack: CoachV6CopyPack) {
    ...
    private func assertSupportContains(_ pack: CoachV6CopyPack, token: String) {
    ...
    private func isAllowedScenarioTransition(
        from previous: CoachV6ScenarioKey,
        to next: CoachV6ScenarioKey
    ) -> Bool {
        if previous == next { return true }
        let forward: Set<CoachV6ScenarioKey> = [
...
enum CoachV6EdgeCaseMatrixPrinter {

    static let logFileURL = URL(fileURLWithPath: "/tmp/WeekFitCoachV6EdgeCaseMatrix.txt")

    static func renderHeader() -> String {
        """
        CoachV6 Edge Case Matrix — ownership snapshot
