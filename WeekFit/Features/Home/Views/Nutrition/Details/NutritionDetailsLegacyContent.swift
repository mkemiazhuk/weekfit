...
struct NutritionDetailsLegacyContent: View {
    ...
    private var header: some View {
        ...
        .overlay(alignment: .bottom) {
            Rectangle()
                .fill(
                    palette.isLight
                        ? WeekFitLightTokens.divider.opacity(0.72)
    ...
    private var noteCard: some View {
        HStack(alignment: .top, spacing: 8) {
            Image(systemName: "info.circle")
                ...
                .foregroundStyle(
                    palette.isLight
                        ? NutritionStyle.nutritionColor.opacity(0.78)
                        : NutritionStyle.nutritionColor.opacity(0.62)
                )
                .padding(.top, 1)
            ...
            Text(WeekFitLocalizedString("nutrition.details.note.full"))
                ...
                .foregroundStyle(
                    palette.isLight
                        ...
                        : WeekFitTheme.whiteOpacity(0.32)
                )
                .lineSpacing(2)
            ...
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(
                    palette.isLight
                        ...
                        : WeekFitTheme.whiteOpacity(0.025)
                )
                .overlay {
                    if palette.isLight {
                        RoundedRectangle(cornerRadius: 16, style: .continuous)
                            .strokeBorder(WeekFitLightTokens.divider.opacity(0.55), lineWidth: 0.8)
                    }
                }
        }
    }
...
private struct NutritionHeroCard: View {
    ...
    var body: some View {
        ...
        .padding(.vertical, 11)
        .nutritionCard(glow: NutritionStyle.nutritionColor, elevated: true)
    }
                ...
                Text("/100")
                    ...
                    .foregroundStyle(
                        palette.isLight
                            ...
                            : WeekFitTheme.whiteOpacity(0.38)
                    )
            }
        }
    }
    ...
    private var hydrationStatus: (text: String, color: Color) {
        guard waterGoal > 0 else {
            return (
                ...
                palette.isLight ? WeekFitLightTokens.textQuaternary : WeekFitTheme.whiteOpacity(0.42)
            )
        }
        ...
        VStack(alignment: .leading, spacing: 11) {
            HStack(spacing: 8) {
                intakePanel(
                    ...
                    softFill: palette.isLight ? WeekFitLightTokens.waterSoft : WeekFitTheme.whiteOpacity(0.026),
                ...
                intakePanel(
                    ...
                    softFill: palette.isLight ? WeekFitLightTokens.nutritionSoft : WeekFitTheme.whiteOpacity(0.026),
                    status: nil,
                    statusColor: nil
                )
            }
            ...
            Rectangle()
                .fill(
                    palette.isLight
                        ? WeekFitLightTokens.divider.opacity(0.65)
            ...
            HStack(spacing: 5) {
                ...
                Text(title)
                    ...
                    .foregroundStyle(
                        palette.isLight
                            ? WeekFitLightTokens.textTertiary
                            : WeekFitTheme.whiteOpacity(0.44)
                    )
                    .lineLimit(1)
                ...
                if let status, let statusColor {
                    Text(status)
                        ...
                        .padding(.vertical, 2)
                        .background(statusColor.opacity(palette.isLight ? 0.14 : 0.12))
                        .clipShape(Capsule())
                        .lineLimit(1)
                        .minimumScaleFactor(0.75)
                }
            ...
            GeometryReader { geometry in
                ZStack(alignment: .leading) {
                    Capsule()
                        .fill(
                            palette.isLight
                                ? WeekFitLightTokens.inactiveTrack
            ...
            VStack(spacing: 0) {
                ...
                Text(String(format: WeekFitLocalizedString("nutrition.details.macro.valueFormat"), Int(value), Int(goal)))
                    ...
                    .foregroundStyle(
                        palette.isLight
                            ? WeekFitLightTokens.textTertiary
                            : WeekFitTheme.whiteOpacity(0.36)
                    )
                    .lineLimit(1)
                    .minimumScaleFactor(0.68)
            }
...
private struct MealTimelineCard: View {
    ...
    private var timelineConnectorColor: Color {
        palette.isLight
            ? WeekFitLightTokens.divider.opacity(0.85)
        ...
        if let visual = PlanTimelineNutritionVisualResolver.resolve(for: meal, customMeals: mealCatalog) {
            ...
            ZStack {
                ...
                Circle()
                    .stroke(
                        palette.isLight
                            ? WeekFitLightTokens.divider.opacity(0.9)
                            : WeekFitTheme.whiteOpacity(0.08),
                        lineWidth: 1
                    )
                    .frame(width: 32, height: 32)
                ...
                Image(systemName: mealTimelineIcon(for: meal))
                    ...
                    .foregroundStyle(
                        palette.isLight
                            ...
                            : WeekFitTheme.whiteOpacity(0.58)
                    )
            }
        }
    }
...
// Dark legacy chrome helpers (NutritionStyle colors live in NutritionDetailsDesign.swift)
...
private extension View {
    func nutritionCard(
        ...
        let emphasis: WeekFitPremiumCardEmphasis
        if elevated {
            emphasis = .elevated
        } else if glow == .clear {
            emphasis = .standard
