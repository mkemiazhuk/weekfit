...
private extension TermsPrivacyView {
        ...
        VStack(alignment: .leading, spacing: 22) {
            ...
            legalSection(
                ...
                paragraphs: [
                    ...
                    "WeekFit uses Firebase Analytics for product usage measurement by default (Share Usage Data in Settings). You can turn this off anytime. Firebase Crashlytics is used for crash diagnostics. These services do not receive raw HealthKit data, derived health or recovery information, sleep status, meal contents, barcodes, coach message text, or other personal health content. See the full privacy policy for details.",
    ...
    var russianLegalCopy: [String: String] {
        [
            ...
            "WeekFit uses Firebase Analytics for product usage measurement by default (Share Usage Data in Settings). You can turn this off anytime. Firebase Crashlytics is used for crash diagnostics. These services do not receive raw HealthKit data, derived health or recovery information, sleep status, meal contents, barcodes, coach message text, or other personal health content. See the full privacy policy for details.": "WeekFit по умолчанию использует Firebase Analytics для измерения использования продукта («Делиться данными об использовании» в настройках). Это можно отключить в любой момент. Firebase Crashlytics используется для диагностики сбоев. Эти сервисы не получают сырые данные HealthKit, производные сведения о здоровье или восстановлении, статус сна, содержимое приёмов пищи, штрихкоды, текст коуча или другой персональный контент о здоровье. Подробности — в полной политике приватности.",
