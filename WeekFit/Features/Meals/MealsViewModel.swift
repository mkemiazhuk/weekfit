...
final class MealsViewModel: ObservableObject {
    ...
    private func recommendationSignature(
        ...
        return [
            ...
            String(format: "%.1f", metrics?.fats ?? -1.0),
            String(format: "%.1f", metrics?.waterLiters ?? -1.0),
            String(format: "%.1f", goals?.calories ?? -1.0),
            String(format: "%.1f", goals?.protein ?? -1.0),
            String(format: "%.1f", goals?.carbs ?? -1.0),
            String(format: "%.1f", goals?.fats ?? -1.0),
            String(format: "%.1f", goals?.waterLiters ?? -1.0),
            activitySignature,
            mealSignature
        ].joined(separator: "#")
    }
