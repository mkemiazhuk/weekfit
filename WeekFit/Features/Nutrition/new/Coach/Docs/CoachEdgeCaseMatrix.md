# Coach Edge Case Matrix — Ownership Conflicts
...
| B′ | 20:15 | same day, auto-focus (no explicit focus) | `session.activity+overlay.stackedRisk` | `activeStrength` | ВНИМАНИЕ | Силовая впереди | Auto-focus picks upcoming core; stacked risk in support only |
...
| Badge | **ВНИМАНИЕ** |
| Today title | Session title (e.g. **Силовая идёт**) — stacked risk in supporting signals |
| Color | `.live` (scenario) + `alertSeverity: .critical` from stacked modifier |

**Conflict note:** Stacked load never replaces scenario copy or Today title — only supporting signals + alert layer.
...
**Rule:** Morning idle must not lead with еда/вода — enforced in `CoachCopyRegistry.shouldSurfaceNutritionSignals`.
...
| + tomorrow hard + completed serious earlier | `session.activity` + `overlay.stackedRisk` | `duringEndurance` | **ВНИМАНИЕ** | **На заезде** |
...
| 8 | during* vs stacked overlay presentation | stacked in support + alert | Scenario title/copy stay session-native |
...
Automated: `CoachEdgeCaseSnapshotTests` — **38 tests** covering §1–§16 (incl. sub-cases).  
Registry guard: `CoachCopyRegistryTests.testMorningReadinessEmptyNutritionSuppressesSupportingSignals`.  
