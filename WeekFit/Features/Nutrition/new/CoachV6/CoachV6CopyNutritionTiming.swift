...
enum CoachV6CopyNutritionTiming {

    /// After ~21:00 — no full-meal advice; sleep and tomorrow's breakfast matter more.
    static func isWindDown(_ timeOfDay: CoachV6TimeOfDay) -> Bool {
        timeOfDay == .lateEvening || timeOfDay == .night
    }

    /// Regular daytime / early evening — normal meal guidance is still appropriate.
    static func isMealWindowOpen(_ timeOfDay: CoachV6TimeOfDay) -> Bool {
        !isWindDown(timeOfDay)
    }

    // MARK: - Supporting signals

    static func fuelBehindSignal(for input: CoachV6CopyBuildInput) -> CoachV6BilingualText {
    ...
    static func hydrationBehindSignal(for input: CoachV6CopyBuildInput) -> CoachV6BilingualText {
        if isWindDown(input.timeOfDay) {
            return .en(
                "Water still behind.",
    ...
    static func fuelCatchUpNextAction(for input: CoachV6CopyBuildInput) -> CoachV6BilingualText {
    ...
    static func hydrationCatchUpNextAction(for input: CoachV6CopyBuildInput) -> CoachV6BilingualText {
        if isWindDown(input.timeOfDay) {
            return .en(
    ...
    static func windDownSleepNextAction() -> CoachV6BilingualText {
        .en(
    ...
    static func fuelCriticalWarning(isActiveSession: Bool) -> CoachV6BilingualText {
        if isActiveSession {
            return .en(
