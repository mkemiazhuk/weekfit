...
enum CoachV6RecoveryBand: String, Equatable, Sendable {
    case good
    case moderate
    case low
}

struct CoachV6DayReadiness: Equatable, Sendable {
    let recoveryPercent: Int
    let sleepHours: Double
    let recoveryBand: CoachV6RecoveryBand
    let hadHeavyYesterday: Bool
    let sleepIsLow: Bool

    static let unknown = CoachV6DayReadiness(
        recoveryPercent: 70,
        sleepHours: 7.5,
...
enum CoachV6DayReadinessResolver {
    ...
    static func resolve(from input: CoachInputSnapshot) -> CoachV6DayReadiness {
    ...
    private static func recoveryBand(for percent: Int) -> CoachV6RecoveryBand {
...
enum CoachV6DayReadinessRouter {

    // MARK: - Idle day stories

    static func idleScenario(for context: CoachV6Context) -> CoachV6ScenarioKey {
    ...
    static func shouldUseProtectTomorrowFresh(_ context: CoachV6Context) -> Bool {
    ...
    static func shouldUseRecoveryAfterHeavyYesterday(_ context: CoachV6Context) -> Bool {
        guard context.sessionPhase == .idle else { return false }
        guard context.activityFamily == .none else { return false }
    ...
    static func shouldUseLowRecoveryPrep(_ context: CoachV6Context) -> Bool {
    ...
    static func lowRecoveryPreSessionScenario(for context: CoachV6Context) -> CoachV6ScenarioKey? {
