...
enum CoachV6ScenarioResolver {
    ...
    static func resolve(_ context: CoachV6Context) -> CoachV6ScenarioResolution {
        let scenario = primaryScenario(for: context)
        let modifiers = CoachV6ScenarioModifiers.from(context: context, scenario: scenario)
        let safetyAlert = safetyAlert(for: context, scenario: scenario)
        return CoachV6ScenarioResolution(
            scenario: scenario,
            modifiers: modifiers,
    ...
    static func primaryScenarioIgnoringNutrition(_ context: CoachV6Context) -> CoachV6ScenarioKey {
        let neutral = CoachV6Context(
    ...
    private static func primaryScenario(for context: CoachV6Context) -> CoachV6ScenarioKey {
    ...
    private static func idleScenario(for context: CoachV6Context) -> CoachV6ScenarioKey {
        CoachV6DayReadinessRouter.idleScenario(for: context)
    }

    // MARK: - Endurance

    private static func enduranceScenario(for context: CoachV6Context) -> CoachV6ScenarioKey {
        switch context.sessionPhase {
        case .pre:
            return CoachV6DayReadinessRouter.lowRecoveryPreSessionScenario(for: context)
                ?? .activeEndurance
        case .during:
    ...
    private static func racketScenario(for context: CoachV6Context) -> CoachV6ScenarioKey {
        switch context.sessionPhase {
        case .pre:
            return CoachV6DayReadinessRouter.lowRecoveryPreSessionScenario(for: context)
                ?? .activeRacket
    ...
    private static func strengthScenario(for context: CoachV6Context) -> CoachV6ScenarioKey {
        switch context.sessionPhase {
        case .pre:
            return CoachV6DayReadinessRouter.lowRecoveryPreSessionScenario(for: context)
                ?? .activeStrength
    ...
    private static func recoveryScenario(for context: CoachV6Context) -> CoachV6ScenarioKey {
    ...
    private static func walkScenario(for context: CoachV6Context) -> CoachV6ScenarioKey {
    ...
    private static func isWalkEveningWindDown(_ context: CoachV6Context) -> Bool {
        context.sessionPhase == .evening ||
            ((context.timeOfDay == .evening || context.timeOfDay == .lateEvening) &&
                context.activityState != .none)
    }

    private static func isWalkRecoveryAction(_ context: CoachV6Context) -> Bool {
    ...
    private static func mindfulRecoveryScenario(for context: CoachV6Context) -> CoachV6ScenarioKey {
        switch context.sessionPhase {
        case .pre:
    ...
    private static func heatScenario(for context: CoachV6Context) -> CoachV6ScenarioKey {
        switch context.sessionPhase {
        case .pre:
