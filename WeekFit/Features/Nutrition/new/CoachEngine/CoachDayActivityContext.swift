...
struct CoachDayActivityContext {
    ...
    var primaryPhase: CoachActivityPhaseV3 { phase }
...
extension CoachActivityContextResolverV3 {
    ...
    static func resolve(
        ...
        if let preparing = preparingCandidate {
            ...
            return CoachDayActivityContext(
                phase: .preparing(
                    activity: preparing,
                    kind: CoachActivityContextResolverV3.kind(for: preparing),
        ...
        if let recent {
            let kind = CoachActivityContextResolverV3.kind(for: recent)
