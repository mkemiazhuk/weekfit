    ...
    categories: [
      {
        ...
        faqs: [
          ...
          { q: "Какие есть вкладки?", a: "«Сегодня» — дневной вывод и кольца, «Коуч» — что важно, «Питание» — учёт еды, «План» — ваша неделя." },
        ],
      },
      {
        icon: "health",
        ...
        faqs: [
          ...
          { q: "Данных нет — что делать?", a: "Откройте Настройки → Конфиденциальность → Здоровье → WeekFit и включите чтение. Сон и восстановление могут появиться через ночь." },
        ],
      },
      {
        icon: "recovery",
      ...
      },
      {
        ...
        title: "Активности",
        faqs: [
          { q: "Как добавляются тренировки?", a: "Завершённые тренировки синхронизируются из Apple Health автоматически. Иногда появляются с небольшой задержкой." },
          { q: "Можно ли запланировать тренировку?", a: "Да — добавьте активности в «План», и Коуч учтёт время, питание и восстановление вокруг них." },
        ],
      },
      {
        icon: "coach",
      ...
      },
      {
        ...
        faqs: [
          ...
          { q: "Как удалить все данные?", a: "WeekFit 1.0 хранит данные на устройстве — удаление приложения стирает локальные план, настройки и предпочтения. Данные Apple Health остаются в приложении «Здоровье»." },
        ],
      },
    ],
  },
    ...
    sections: [
      ...
      },
      {
        ...
        blocks: [
          ...
          { t: "p", v: "В соответствии с требованиями Apple HealthKit данные из HealthKit используются только для работы функций WeekFit. Они никогда не применяются для рекламы или маркетинга и не продаются и не передаются третьим лицам или брокерам данных." },
      ...
      },
      {
        ...
        blocks: [{ t: "p", v: "WeekFit даёт рекомендации по фитнесу и wellness, но не является медицинским устройством и не заменяет консультацию врача." }],
      },
      {
        id: "contact",
        h: "Контакты",
...
> = {
  ...
  ru: {
    ...
    sections: [
      ...
      { id: "health", h: "3. Медицинский дисклеймер", blocks: [{ t: "p", v: "WeekFit даёт рекомендации по фитнесу и wellness. Это не медицинское устройство и не заменяет консультацию врача. Перед началом или изменением тренировок, питания или восстановления проконсультируйтесь со специалистом." }] },
...
> = {
  ...
  ru: {
    ...
    roadmap: "Впереди больше — облачная синхронизация, глубокие инсайты и Apple Watch в планах.",
    releases: [
      {
        version: "1.0",
        date: "2026",
...
> = {
  ...
  ru: {
    ...
    boilerShort: "WeekFit — спокойный AI-коуч, который читает сон, активность, питание и восстановление из Apple Health и говорит, что важно именно сегодня.",
    boilerLong: "WeekFit — спокойный AI-коуч для iOS. Вместо очередной панели с цифрами он интерпретирует связи между сном, активностью, питанием и восстановлением — на основе Apple Health и с обработкой на устройстве — чтобы объяснить, что важно сегодня. Приватный по умолчанию: без аккаунта и без использования данных о здоровье для рекламы.",
    colorsTitle: "Цвета бренда",
    assetsTitle: "Материалы для загрузки",
    assetsNote: "Иконка приложения и скриншоты. По остальному — напишите нам.",
...
> = {
  ...
  ru: {
    ...
    include: ["Версию iOS и модель iPhone", "Что ожидали и что произошло", "Скриншот, если это уместно"],
    cta: "support@weekfit.app",
  },
};
...
> = {
  ...
  ru: {
    ...
    lead: "WeekFit скоро появится в App Store. На основе Apple Health, приватный по умолчанию.",
    soon: "Скоро",
    reqTitle: "Требования",
    requirements: [
    ...
    qr: "Отсканируйте, чтобы открыть на iPhone (скоро)",
  },
};
